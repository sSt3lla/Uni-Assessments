...
public class DatabaseFileReader {
	...
	public Database read() throws InvalidRowException, DuplicateKeyException {		
		...
		return new DatabaseImpl(schema, rows, keyField);
	}
	...
	private int findKeyField(String schemaLine) {
		int index = 0;
		for (String pair : schemaLine.split(",")){

			//The name and type
			String name = pair.split(":")[0];
			String type = pair.split(":")[1];
			if (type.endsWith("*"))
				return index;
			index++;
		}
		throw new RuntimeException("Couldn't find key field");
	}
}
